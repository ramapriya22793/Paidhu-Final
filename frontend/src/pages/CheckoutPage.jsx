...
const CheckoutPage = () => {
  ...
  // Fetch summary when cart changes and fire InitiateCheckout Meta Pixel event
  useEffect(() => {
    if (cart.length > 0) {
      fetchSummary();
  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      ...
      setErrorMsg(error.message || "Failed to process order. Please try again.");
      setSubmitting(false);
      isSubmittingRef.current = false;
    }
  };
      ...
      <div className="max-w-[1300px] mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start mt-6">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="block lg:hidden bg-[#220919] rounded-[2rem] p-6 mt-4 shadow-xl border border-[#d4af37]/20">
              <div className="flex justify-between items-center mb-4">
                <span className="text-white/70 font-bold text-xs uppercase tracking-wider">Grand Total</span>
                <span className="text-2xl font-black text-[#d4af37]">
                  {(loadingSummary || isSummaryOutdated) ? (
                    <span className="text-sm text-white/40 italic">Calculating...</span>
                  ) : (
              ...
              <motion.button
                ...
                className={`w-full text-white py-4 rounded-full flex items-center justify-center gap-2 font-bold text-sm transition-all duration-300 ${
                  (submitting || loadingSummary || isSummaryOutdated)
                    ? 'bg-gray-600 cursor-not-allowed opacity-50 shadow-none'
                    : 'bg-gradient-to-r from-[#662654] to-[#7f3069] shadow-[0_8px_30px_rgba(102,38,84,0.3)]'
                }`}
              >
                <span>
                  {submitting 
          ...
          <div className="lg:col-span-5 lg:sticky lg:top-32 space-y-6">
            ...
            <div className="bg-[#220919] text-white rounded-[2.5rem] p-6 md:p-8 shadow-2xl relative overflow-hidden border border-[#d4af37]/20">
              ...
              {/* Place Order Button */}
              <div className="pt-6 relative z-10 hidden lg:block">
                <motion.button
                  type="button"
                  onClick={handlePlaceOrder}
                  disabled={submitting || loadingSummary || isSummaryOutdated}
