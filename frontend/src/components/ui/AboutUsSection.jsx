...
const defaultData = {
  ...
  communityCta: {
    ...
    illustration: 'https://images.unsplash.com/photo-1564890369478-c89ca6d9cde9?q=80&w=1000&auto=format&fit=crop'
  }
};
  ...
  if (loading) {
    return (
      <div className="w-full h-screen flex flex-col items-center justify-center bg-[#faf9f7] gap-4">
        ...
        <p className="text-[#662654] font-medium animate-pulse">Loading Our Story...</p>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 gap-16">
            {data.founders.list?.map((founder, i) => (
              <div 
                key={founder.id || i} 
                className={`flex flex-col md:flex-row gap-8 items-center md:items-start p-6 md:p-8 rounded-[2rem] border border-gray-50 bg-gray-50/50 hover:bg-white hover:shadow-lg transition-all duration-300 ${
                  i % 2 === 1 ? 'md:flex-row-reverse' : ''
                }`}
              >
                {/* Photo */}
                <div className="w-48 h-48 rounded-full overflow-hidden shrink-0 shadow-lg relative border-4 border-white bg-gray-100">
                  <img
                    src={founder.image}
                    alt={founder.name}
                    className="absolute inset-0 w-full h-full object-cover"
                ...
                <div className="space-y-4 flex-1">
                  <h3 className="text-2xl font-bold text-[#5a2141] font-serif border-b border-[#662654]/10 pb-2">{founder.name}</h3>
                  <div className="text-gray-600 font-medium text-sm md:text-base leading-relaxed space-y-4 text-justify">
        ...
        <div className="bg-gradient-to-br from-[#662654] to-[#501b41] rounded-[2.5rem] p-8 md:p-12 text-white shadow-xl relative overflow-hidden grid grid-cols-1 lg:grid-cols-12 items-center gap-8">
          ...
          <div className="lg:col-span-4 h-[220px] rounded-[1.5rem] overflow-hidden relative shadow-lg z-10">
            <img
              ...
              className="absolute inset-0 w-full h-full object-cover"
              onError={(e) => {
                e.target.src = 'https://images.unsplash.com/photo-1564890369478-c89ca6d9cde9?q=80&w=1000&auto=format&fit=crop';
              }}
            />
          </div>
        </div>
