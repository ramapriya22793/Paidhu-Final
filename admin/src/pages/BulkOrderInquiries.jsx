...
const BulkOrderInquiries = () => {
  ...
  const fetchInquiries = async () => {
    try {
      ...
      const res = await axios.get((import.meta.env.VITE_API_URL || 'https://paidhu-final-anm2.vercel.app') + '/api/bulk-orders', config);
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      const config = { headers: { Authorization: `Bearer ${authService.getToken()}` } };
      await axios.patch(`${import.meta.env.VITE_API_URL || 'https://paidhu-final-anm2.vercel.app'}/api/bulk-orders/${id}/status`, { status: newStatus }, config);
  ...
  const deleteInquiry = async (id) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${authService.getToken()}` } };
      await axios.delete(`${import.meta.env.VITE_API_URL || 'https://paidhu-final-anm2.vercel.app'}/api/bulk-orders/${id}`, config);
      fetchInquiries(); // Refresh
    } catch (error) {
