    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-20">
      ...
      <div className="flex overflow-x-auto gap-2 bg-white p-2 rounded-xl shadow-sm border border-gray-100">
        {[
          ...
          { id: 'learn', label: 'Learn (Experts)', icon: FiBookOpen },
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
        ...
        {activeTab === 'learn' && (
          <div className="space-y-6">
            <h2 className="text-xl font-bold text-brand-plum border-b pb-2 mb-6">LEARN Section (Experts)</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {data.learn.map((item, index) => (
                <div key={item.id} className="bg-gray-50 p-4 rounded-xl border border-gray-200 space-y-4">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">Expert Name</label>
                    <input type="text" value={item.name} onChange={(e) => handleArrayChange('learn', index, 'name', e.target.value)} className="w-full border rounded-lg px-4 py-2 font-bold uppercase" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">Title/Role</label>
                    <input type="text" value={item.title} onChange={(e) => handleArrayChange('learn', index, 'title', e.target.value)} className="w-full border rounded-lg px-4 py-2 uppercase" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">Image URL</label>
                    <input type="text" value={item.image} onChange={(e) => handleArrayChange('learn', index, 'image', e.target.value)} className="w-full border rounded-lg px-4 py-2" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">Description Text</label>
                    <textarea value={item.text} onChange={(e) => handleArrayChange('learn', index, 'text', e.target.value)} className="w-full border rounded-lg px-4 py-2" rows={3}></textarea>
                  </div>
        ...
        {activeTab === 'highlights' && (
          <div className="space-y-6">
            <h2 className="text-xl font-bold text-brand-plum border-b pb-2 mb-6">HIGHLIGHTS Video Banner</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">Video Thumbnail Image URL</label>
                ...
                <div>
                  ...
                  <input type="text" value={data.highlights.videoUrl} onChange={(e) => setData({...data, highlights: {...data.highlights, videoUrl: e.target.value}})} className="w-full border rounded-lg px-4 py-2" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Preview</label>
